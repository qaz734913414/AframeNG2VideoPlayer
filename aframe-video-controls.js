...
//text that displays under the player
	...
	AFRAME.registerComponent('video-controls', {
	  ...
	  init: function () {
	    ...
	    // Next two vars used to control transport bar with keyboard arrows
			//total number of steps (kind of like scenes in a movie) you can skip to the next scene
	    this.bar_steps = 10.0;
	    this.current_step = 0.0;

	    this.el.setAttribute("visible", true);

			//grabs data "src" which is the video we put in and returns the element from the html
	  ...
	  tick: function (t) {
	    ...
	    if(typeof(this.last_time) === "undefined" || (t - this.last_time ) > 250) {
	        ...
	        if(this.video_el.readyState > 0) {
	            ...
	            //Display how much of the video has buffered
	            if (this.video_el.buffered.length > 0) {

	                // Synchronize current step with currentTime
									//percentage of video done*number of pixels to light up accordingly
